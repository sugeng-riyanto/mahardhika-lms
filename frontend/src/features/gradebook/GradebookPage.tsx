...
import { BarChart3, Search, Award, Eye, EyeOff } from 'lucide-react'
              ...
              <div className="flex items-center gap-3">
                <div className="w-24 bg-navy-800 rounded-full h-1.5" role="progressbar" aria-valuenow={grade.percentage} aria-valuemin={0} aria-valuemax={100} aria-label={`${grade.activity_title}: ${grade.percentage}%`}>
                  <div className={`h-1.5 rounded-full ${
                    grade.percentage >= 80 ? 'bg-green-500' : grade.percentage >= 60 ? 'bg-yellow-500' : 'bg-red-500'
...
export function GradebookPage() {
  ...
  // Student/parent view: simplified summary
  ...
  // Instructor/Admin/Owner view: full gradebook table
    ...
    <div className="page-container">
      ...
      <div className="flex flex-col sm:flex-row gap-4 mb-6">
        ...
        <button
          ...
          aria-label={showUnreleased ? 'Showing all grades including unreleased' : 'Showing released grades only'}
        >
                  ...
                  <tr key={grade.id} className="border-b border-navy-800 hover:bg-navy-800/50 transition-colors">
                    ...
                    <td className="px-4 py-3 text-center">
                      <span className="text-xs text-navy-400 capitalize">
                        {grade.activity_type?.replace('_', ' ') || '—'}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-center">
                      <span className={`text-sm font-mono ${getGradeColor(grade.percentage)}`}>
                        {grade.score}/{grade.max_score}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-center">
                      <div className="flex items-center justify-center gap-2">
                        <div className="w-16 bg-navy-800 rounded-full h-1.5" role="progressbar" aria-valuenow={grade.percentage} aria-valuemin={0} aria-valuemax={100} aria-label={`${grade.student_name} ${grade.activity_title}: ${grade.percentage}%`}>
                          <div className={`h-1.5 rounded-full ${
                            grade.percentage >= 80 ? 'bg-green-500' :
                            grade.percentage >= 60 ? 'bg-yellow-500' : 'bg-red-500'
                          }`} style={{ width: `${Math.min(grade.percentage, 100)}%` }} />
                        </div>
                        <span className={`text-sm font-bold font-mono ${getGradeColor(grade.percentage)}`}>
                          {grade.percentage}%
                        </span>
                      </div>
                    </td>
          ...
          {filteredGrades.length === 0 && (
            <div className="py-12 text-center">
              ...
              <p className="text-navy-400">
                {searchQuery ? 'No grades match your search.' : 'No grades found.'}
              </p>
              <p className="text-navy-500 text-sm mt-1">
                Grades appear here once instructors create and release them.
              </p>
            </div>
          )}
        </div>
